...
public class HubGameService : MonoBehaviour
{
    ...
    private void SyncScoreManager()
    {
        ...
        for (int i = 1; i <= 4; i++)
        {
            ...
            {
                manager.ApplyAssignment(i - 1, string.Empty, string.Empty, 0);
            }
        }
    }
    ...
    private GameResultRequest BuildResultPayload(IReadOnlyDictionary<string, int> scores)
    {
        ...
        payload.startTime = (gameStartTimeUtc ?? DateTime.UtcNow).ToUniversalTime().ToString("o", CultureInfo.InvariantCulture);
        payload.results = entries.Count > 0 ? entries.ToArray() : Array.Empty<ResultEntry>();
        return payload;
    }
