...
public class TargetPersonaBinder : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        string slotId;
        if (!TargetHubInputDriver.TryResolveControllerId(targetComponent.ptype, out slotId))
        {
            ResetToDefault();
            return;
        }
    ...
    private PersonaType ResolvePersonaType(string personality)
    {
        ...
        int value;
        if (!int.TryParse(personality, NumberStyles.Integer, CultureInfo.InvariantCulture, out value))
        {
            return PersonaType.Unknown;
        }
